-- seed.sql
-- PostgreSQL seed data for users, projects, tasks
-- Safe order: users -> projects -> tasks
-- Run with: psql -U postgres -d database_name -f seed.sql

BEGIN;

-- Optional reset for deterministic seed in development.
-- Keeps FK integrity by truncating in dependency-safe order with CASCADE.
TRUNCATE TABLE tasks, projects, users RESTART IDENTITY CASCADE;

-- -----------------------------------------------------------------------------
-- 1) USERS
-- Notes:
-- - email values are unique and also unique when lower-cased (ux_users_email_lower).
-- - password_hash uses valid bcrypt hash string.
-- - created_at uses NOW().
-- -----------------------------------------------------------------------------
...
-- 2) PROJECTS
-- Notes:
-- - owner_id is resolved from users by email to guarantee valid FK.
-- - each project belongs to a real owner.
-- -----------------------------------------------------------------------------
